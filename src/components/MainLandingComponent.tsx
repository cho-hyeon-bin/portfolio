...
import '@/sass/component/MainLandingComponent.sass';
const MainLandingComponent: React.FC = () => {
	const netflixIntroRef = useRef<HTMLDivElement>(null);
	useEffect(() => {
		const timer = setTimeout(() => {
			if (netflixIntroRef.current) {
				netflixIntroRef.current.style.display = 'none';
			} else {
				console.log("test")
			}
		}, 5000);
